    ...
    <div class="exam-body">
      <div class="exam-content-wrapper">
        ...
        <main class="exam-main" id="examMain">
            ...
            ?>
            <div class="soal-card <?= $no === 1 ? 'soal-card--active' : '' ?>"
                 id="soal-<?= $no ?>"
                 data-no="<?= $no ?>"
                 data-id="<?= htmlspecialchars($id_soal) ?>">
                ...
                <div class="soal-opsi">
                    <?php foreach ($opsi as $key => $teksOpsi):
                        if ($teksOpsi === null || $teksOpsi === '') continue;
                        $selected = ($jawabanDipilih === $key);
                    ?>
                    <label class="opsi-item <?= $selected ? 'opsi-item--selected' : '' ?>"
                           data-key="<?= $key ?>">
                        <input type="radio"
                               name="jawaban_<?= $no ?>"
                               value="<?= $key ?>"
                ...
                <div class="soal-nav">
                    <button class="btn-prev poppins-semibold"
                            onclick="pindahSoal(<?= $no - 1 ?>)"
                            <?= $no === 1 ? 'disabled' : '' ?>>
                        <i class="ph ph-caret-left"></i> Sebelumnya
                    </button>

                    <button class="btn-next poppins-semibold"
                            onclick="pindahSoal(<?= $no + 1 ?>)"
                            <?= $isLast ? 'disabled' : '' ?>>
            ...
            <div class="exam-sidebar__card">
                ...
                <div class="nav-grid" id="navGrid">
                    <?php foreach ($soalList as $idx => $soal):
                        ...
                        if ($no === 1) $cls .= ' nav-btn--current';
                    ?>
                    <button class="nav-btn poppins-semibold <?= trim($cls) ?>"
                            id="navBtn-<?= $no ?>"
                ...
                <div class="exam-warning">
                    ...
                    <p class="poppins-regular">Jangan refresh atau pindah tab. Aktivitas akan tercatat oleh pengawas.</p>
                </div>
            </div>
        </aside>
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        console.warn('[EXAM] Siswa berpindah tab/window');
    }
});
